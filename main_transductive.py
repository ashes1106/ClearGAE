...
def pretrain(model, graph, feat, A, nei_simi, recon_infor_low, recon_infor_high, optimizer, max_epoch, device, scheduler, num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob, i, logger=None):
...
def main(args):
    ...
    for i, seed in enumerate(seeds):
        ...
        if not load_model:
            model = pretrain(model, graph, x, A, nei_simi,recon_infor_low, recon_infor_high, optimizer, max_epoch, device, scheduler,num_classes, lr_f, weight_decay_f, max_epoch_f, linear_prob, i, logger)
            model = model.cpu()
        end_time = time.time()
...
if __name__ == "__main__":
    args = build_args()
    if args.use_cfg:
        args = load_best_configs(args, "/mmu_nlp_ssd/chenge03/graph/KR/bridge_map/configs.yml")
    print(args)
    main(args)
